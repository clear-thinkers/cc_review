 ...
 *
 * This is a *read-only* dev-database lookup used by the add-ingredient
 * skill to resolve "the food" before anything is generated or written.
 * It always reads from .env.local (dev) and never accepts a --prod flag --
 * this skill must never touch production.
 *
 * Usage:
 *   node find-recipe.mjs "milkshake"
...
async function main() {
  ...
  const { data, error } = await supabase
    .from("shop_recipes")
    .select(
      "id,slug,title,title_i18n,base_ingredients_i18n,special_ingredient_slots_i18n,food_type,cook_method"
    )
