 ...
 * this script must never *connect to* production. On a successful --apply
 * it additionally writes a scoped migration file to supabase/migrations/
 * containing the same two statements, ready for this repo's normal
 * db:push:prod flow -- writing that SQL to disk is not the same as running
 * it, so this stays consistent with "never touches prod directly."
 *
 * Usage:
 ...
 *
 * Pass --no-prod-migration to skip writing the supabase/migrations/ file on
 * --apply (e.g. for a dev-only experiment that isn't going to prod).
 *
 * Pass --prod-verified if you've separately confirmed production's current
 * ingredient list for this recipe matches what dev had when this ran --
 * this only changes the comment written into the migration file, it does
 * not change what SQL is generated.
 */
...
const BOOLEAN_FLAGS = new Set(["apply", "force", "no-prod-migration", "prod-verified"]);

function parseArgs(argv) {
  const args = { apply: false, force: false, "no-prod-migration": false, "prod-verified": false };
...
function writeProdMigrationFile({ ingredientKey, recipeSlug, slot, ingredientUpsertSql, recipeUpdateSql, prodStateVerified }) {
  ...
  const caveat = prodStateVerified
    ...
    : [
        `-- NOT verified against production's current state for slug = '${recipeSlug}'.`,
  ...
  const sql = `-- Add the "${ingredientKey}" ingredient and link it into "${recipeSlug}"'s
-- ${slot} ingredients.
--
...
-- shop_recipes.${slot === "base" ? "base_ingredients" : "special_ingredient_slots"} /
-- ${slot === "base" ? "base_ingredients_i18n" : "special_ingredient_slots_i18n"} for slug = '${recipeSlug}' --
...
where slug = ${escapeSqlLiteral(recipeSlug)};`;
  ...
  if (!args["no-prod-migration"]) {
    const migrationPath = writeProdMigrationFile({
      ...
      ingredientUpsertSql,
      recipeUpdateSql,
      prodStateVerified: args["prod-verified"],
    });
    console.log(
