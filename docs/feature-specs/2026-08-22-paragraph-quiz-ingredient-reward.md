...
- [x] Live-QA checklist (see Risks/Test plan) completed against a dev Supabase project **through the actual app UI**, 2026-08-23 — an actual browser session completed a paragraph quiz and the reward panel rendered correctly.
...
## Addendum — 2026-08-24: broadened to all packaged sessions

The Scope section above already called this out as the expected next step: *"Scoped to the paragraph quiz only, but `reward_random_ingredients` is parameterized generically by `quiz_session_id` (not paragraph-specific) — extending this to the classic character/phrase fill-test later would be an additional call site, not a redesign."* That extension shipped 2026-08-24, exactly as an additional call site — **no migration, no RPC change, no new route** (`AI_CONTRACT.md §2` scope boundaries: none triggered).

**What changed:**
- `moveQuizForward` in `src/app/words/shared/words.shared.state.ts` (the character/phrase fill-test completion path, used by `FillTestReviewSection.tsx`) now calls `rewardRandomIngredients(quizSessionId)` right after `recordQuizSession` succeeds, but **only when the session is a packaged review test session** (`completedReviewTestSession` non-null — i.e. `requestedReviewTestSessionId && activeReviewTestSession`). An ad-hoc (non-packaged) due-review completion is unaffected: no reward call, no schema/behavior change, matching this feature's "packaged session" framing exactly (an explicit product decision, not an oversight — ad-hoc quizzes were never in scope).
- The completion logic was split into two phases (`finalizeQuizCompletion` + the new `continueQuizAfterIngredientReward`), mirroring `ParagraphQuizReviewSection.tsx`'s existing `finishSession`/`completeSessionAndReturn` split: a non-empty reward result defers `completeReviewTestSession`/session finalization until the child dismisses the reward panel; an empty result or a failed reward call falls straight through to finalization unchanged, exactly as today.
- The reward panel UI and `buildRewardHeadline` were extracted out of `ParagraphQuizReviewSection.tsx` into a new shared component, `src/app/words/review/IngredientRewardPanel.tsx`, used by both `FillTestReviewSection.tsx` and `ParagraphQuizReviewSection.tsx` — avoids duplicating the panel visuals/strings now that a second call site exists. `buildRewardHeadline` moved to `src/app/words/review/reviewSession.utils.ts` (already shared between both review flows).
- Strings moved from `paragraphQuiz.reward` to a new shared top-level `ingredientReward` section in `words.strings.ts` (EN + ZH), since the copy was always generic ("Quiz complete! You earned N ingredients...") and never paragraph-specific.
- A mixed (character + phrase) packaged session is covered the same way as a character-only or phrase-only one — the reward call keys off "is this a packaged session," not target composition.

**Unchanged:** the RPC (`reward_random_ingredients`), the `shop_ingredient_rewards` table, RLS, idempotency behavior, the empty-pool/failure-never-blocks-completion guarantee, and every acceptance criterion above — all still hold, now exercised from a second call site.
