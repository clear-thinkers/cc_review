...
| I | Article import → known/unknown triage | **Most immediate Tier 1 need. Depends on item D shipping first.** Parent pastes a multi-paragraph article; app renders it broken into paragraphs in readable form with individual characters *and phrases* visually flagged known vs. unknown against the family's `words` table. Parent selects specific unknown characters and/or multi-character phrase spans (new select/highlight interaction — no existing precedent) and bulk adds + tags the selection in the same flow. Character-diff logic reuses existing ingestion (`extractUniqueHanzi`, `computeIngestionResult` in `src/app/words/add/`); phrase-diff logic is new and requires D's phrase storage to exist first. Selection UI is net-new — no existing text-span-selection component in the app; the per-character `<span>`-splitting in `renderRubyLine` (`src/app/words/review/flashcard/FlashcardCard.tsx`) is reusable as a rendering primitive only, not the selection interaction itself. Open question for spec: extend `/words/add` in place vs. a new route (new-route boundary per `AI_CONTRACT.md §2`) | TBD | 📋 Planned |
