...
38. **Reveal after 3 bounces** (feature spec `2026-08-22-paragraph-quiz-reveal-after-bounces.md`, shipped 2026-08-22, corrected twice same day after live QA): once a **word-bank item** has itself been dragged onto a wrong blank 3 times, cumulative across every blank it was tried on (not the blank pill in the paragraph text — fix 1; not `retryCount`, which counts wrong attempts AT a target blank regardless of which item caused them — fix 2), that bank item switches to a purple style and a "Hint"/提示 badge appears next to it. This is tracked by a second, independent, persisted counter on `ParagraphQuizProgressData` — `wrongDragCounts`, keyed by the DRAGGED item's own `spanId` — bumped alongside (never replacing) `blankState[targetSpanId].retryCount` on every wrong drop; `retryCount` still exclusively drives `deriveParagraphBlankTier` grading, `wrongDragCounts` drives only reveal eligibility. `wrongDragCounts` is optional in `isParagraphQuizProgressData`'s validation, defaulting to `{}` on resume of a progress row saved before this field existed. Clicking the badge opens a read-only popup (`ParagraphQuizRevealPopup.tsx`) — flashcard-style content (hanzi, pinyin, meanings, `include_in_fill_test` phrases/examples, **every** matching `flashcard_contents` pronunciation entry shown stacked, never guessed) for a character-backed item, or phrase/pinyin/meaning/**first** `include_in_fill_test` example for a phrase-backed item — sourced entirely from already-loaded `allFlashcardContents`/`vocabPhrases`, no new fetch. The popup never calls `gradeWord`/`gradeVocabPhrase`/`nudgeWordFamiliarity`/`setSelectedBankSpanId` and never mutates `blankState`/`retryCount`/coins; the blank still requires a correct drag/drop placement to complete the page. No new column, table, RPC, or route — `wrongDragCounts` lives inside the existing jsonb `progress_data` alongside `blankState`. Only one popup is open at a time; opening a second bank item's badge replaces the first popup's content rather than stacking.
39. **Ingredient reward broadened to every packaged session (2026-08-24)** — the ingredient-reward mechanic introduced for paragraph-quiz sessions (rule set above; `docs/feature-specs/2026-08-22-paragraph-quiz-ingredient-reward.md`) now also fires on completion of a **packaged** character, phrase, or mixed review test session (rules 1–27 above) — not an ad-hoc (non-packaged) due-review completion, which is unaffected. `moveQuizForward` in `words.shared.state.ts` calls the same generic `rewardRandomIngredients(quizSessionId)` right after `recordQuizSession` succeeds, gated on `completedReviewTestSession` being non-null (i.e. a packaged session); a non-empty result defers `completeReviewTestSession`/finalization until the child dismisses the reward panel (new `finalizeQuizCompletion`/`continueQuizAfterIngredientReward` split, mirroring the paragraph-quiz path's existing `finishSession`/`completeSessionAndReturn` split), an empty result or a failed call falls straight through unchanged. No migration, RPC, or route change — an additional call site only, exactly as the original spec's Scope section anticipated. The reward panel UI (`IngredientRewardPanel.tsx`) and its strings (`str.ingredientReward`, moved out of `paragraphQuiz.reward`) are now shared between `FillTestReviewSection.tsx` and `ParagraphQuizReviewSection.tsx` rather than paragraph-quiz-only.
...
17. **Added 2026-08-23 (roadmap item F, feature spec `2026-03-30-shop-ingredient-shopping.md`) — ingredient shopping:** the ingredient detail sub-modal (opened by tapping a base or special ingredient inside an unlocked recipe's modal) shows the ingredient's per-unit coin cost, a quantity stepper (+/− and direct numeric entry, default 1, clamped to `SHOP_INGREDIENT_QUANTITY_MIN`–`MAX`), and a Buy button that spends `costPerUnit × quantity` coins in one tap. A deliberate departure from the original draft spec's one-unit-per-tap design — bulk purchase was the explicit ask, and the ledger shape (one row per unit, see Data Schema) supports it with no redesign.
18. Buy UI is shown only for an ingredient that resolves to a real shared catalog entry (`resolveShopIngredientCatalogEntry` finds a `shop_ingredient_prices` row) — an ingredient with no resolvable `ingredientKey` shows the existing read-only icon/name/quantity-needed view with no purchase controls, same as before this change.
19. Purchasing is persisted through the `purchase_shop_ingredient` RPC only; it is atomic (wallet `FOR UPDATE` lock, same pattern as `unlock_shop_recipe`) and requires the target recipe to already be unlocked by the caller — enforced both by the UI (the sub-modal is only reachable from an unlocked recipe) and server-side by the RPC.
20. Ingredient purchases append to `shop_coin_transactions` under `action_type = 'purchase_ingredient'` (one summary row per buy action, `coins_spent` = the full multi-unit total), so the existing coin breakdown panel's "Spent" line (renamed from "Spent on Recipes", since it now also includes ingredient spend) and the coin history modal both surface ingredient spend alongside recipe unlocks without a separate line item.
21. Each purchased unit also appends one row to `shop_ingredient_purchases` (one row per unit, not per buy action — matching `shop_ingredient_rewards`/`shop_ingredient_consumptions`'s existing ledger convention), which feeds into Shop Kitchen's cupboard availability alongside quiz rewards — see Shop Kitchen Rule 10.
22. Out of scope for this pass (may follow later): a dedicated ingredient inventory page, a "recipe ready to cook" visual badge on the recipe wall/modal (v1's readiness concept from the original draft spec — distinct from Shop Kitchen's own, already-shipped `computeShopCookReadiness`), and purchasing special (non-base) ingredients beyond whatever the shared catalog already prices.
...
10. **Added by Shop Kitchen (2026-08-23):** Recipe saves may also set `cook_method` (`null` / `'stove'` / `'oven'`) and **`food_type`** (`null` / `'drinks'` / `'hotmeal'` / `'desserts'`) on the recipe form. A recipe defaults to both `null` (not cookable, unsortable) until a platform admin opts it in; validation requires `food_type` whenever `cook_method` is set, since the shelf has no "unsorted" fallback tab.

### Shop Kitchen Rules (`/words/shop/kitchen`)

These rules govern the child-facing cooking page added 2026-08-23 (`docs/feature-specs/2026-08-23-kitchen-page.md`), revised same day once a real kitchen illustration (`public/kitchen/full-kitchen.png`) replaced the original card-grid design with clickable regions over the artwork, and child-driven shelf drag-and-drop was replaced with admin-preassigned food types — see the spec's Revision section for the full before/after:
...
2. The page renders `full-kitchen.png` as its scene background with five clickable regions over it: the fridge (ingredients), the recipe book (recipe selection), the top half of the stove/oven appliance (stovetop), its bottom half (oven), and the shelf unit. There is no card-grid fallback layout.
...
6. **A dish's shelf category is never a per-dish choice.** Which of the shelf's three tabs (Drinks/Hot Meal/Desserts) a shelved dish displays under is derived at read time from the dish's *recipe's* `food_type` (`buildShelfTilesByFoodType` in `kitchen.types.ts`), not stored on the dish row and never set by the child.
7. Moving dishes from the countertop to the shelf is persisted through the `organize_shop_kitchen_countertop` RPC only — no parameters, no per-dish variant. It bulk-updates every one of the caller's `location = 'countertop'` rows to `'shelf'` in a single call and returns the count moved.
8. A newly-cooked dish always lands with `location = 'countertop'`, regardless of where any existing dishes of that same recipe currently sit (countertop or shelf). Re-cooking an already-shelved recipe bumps the count of the dish wherever it currently sits — it is never reset back to the countertop.
9. No cap exists on how many times a recipe may be cooked, subject only to ingredient availability and countertop capacity (which Organize clears, not a hard ceiling on cooking itself).
10. Cooking spends ingredients only — it never spends or touches coins, `wallets`, or `shop_coin_transactions`. This is a deliberate second, orthogonal economy from the existing coin-unlock system.
...
| `variant_icon_rules` | jsonb | Variant icon rules keyed by ingredient-match combinations. Each rule may also carry an optional `titleI18n: { en, zh }` (added 2026-08-24, Shop Kitchen named variants -- see Shop Kitchen Rule 13) giving that exact combination its own display name; absent/blank falls back to the recipe's own title |
...
| `food_type` | text (nullable) | `'drinks'` \| `'hotmeal'` \| `'desserts'` \| `null` — added by Shop Kitchen (2026-08-23, same-day revision). Which shelf tab a cooked dish is organized under; Shop Admin requires this whenever `cook_method` is set |
| `created_at` | timestamptz | Server timestamp |
| `updated_at` | timestamptz | Server timestamp |
...
| `location` | text | `'countertop'` \| `'shelf'`; default `'countertop'`. Which shelf *tab* a shelved dish displays under is derived at read time from the dish's recipe's `food_type` (see below) — never stored on this row |
| `special_ingredient_keys` | jsonb | Added 2026-08-24. Array of ingredient keys spent on this cook beyond the recipe's base ingredients; default `'[]'`. Raw fact only — which icon the dish displays is resolved at read time from this array against the recipe's `variant_icon_rules` (`resolveShopRecipeIconPath`), never stored |
| `cooked_at` | timestamptz | Server timestamp |
| **RLS Guarantee** | | Family-scoped read; **no direct client insert/update/delete policy at all** — `cook_shop_recipe` is the only inserter, `organize_shop_kitchen_countertop` is the only updater (and only bulk-moves `location` from `'countertop'` to `'shelf'`, never per-dish) |

`location` is the one field on any shop-adjacent table in this codebase that mutates post-insert — a deliberate, called-out departure from the append-only convention everywhere else (see the feature spec's Risks section), because organizing the countertop is inherently a mutation, not an event log.
