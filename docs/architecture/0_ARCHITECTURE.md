...
10. **Updated 2026-08-23:** Ingredient availability shown in the Cupboard reflects both `shop_ingredient_rewards` (quiz rewards) and `shop_ingredient_purchases` (roadmap item F, shipped 2026-08-23) — the two reward-like ledgers are concatenated client-side (`[...rewards, ...purchases]`) before being passed into the unchanged `buildShopIngredientAvailabilityMap(additions, consumptions)`, exactly the "second reward-like input, not a redesign" plan this rule originally called for.
