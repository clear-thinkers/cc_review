...
28. Actions column is **Delete only**, immediate with no confirmation dialog (matching the Characters view's no-content-cascade delete path) — no Reset button, since phrases carry no scheduling counters to reset.
